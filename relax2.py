...
for m in range(len(modelist)):
  for n in range(len(modelist)):
    ...
    if m >= n:
      continue;
    modeproject[modelist[m]]=0.0;
    modeproject[modelist[n]]=0.0;
    dp=np.zeros(3*natom);
    mass3D=[];
    ...
    positdiff.writeinterpolate(natom,axis,dp2D+ExpandPosition,"./phase/cubic","./RELAX2/Abinit"+str(modelist[m])+"_"+str(modelist[n]));
